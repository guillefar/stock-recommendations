...
- `recommendation_outcomes` — one row per recommendation per horizon (7d/30d/90d/180d/365d; grading bands widen with the horizon **and scale to the asset class** — ETF bands are 0.30× the single-stock bands since session 26; 30d is the headline metric); grades the forward return. Populated separately by `python -m src.evaluate_outcomes`, not by the main pipeline.
