...
Pipeline is wired end-to-end. **Wave 2 is merged and Wave 3 is complete** as of session 09 (2026-06-29, branch `feat/session-09-wave-3`, pushed not merged). Session 08's Wave 2 work was merged to `main` (`b4af216`) before this session, so the single daily cron, the per-phase action enum, and structured ticker output are all in production. **Post-merge verification (read-only):** since 2026-06-15 exactly one batch of 63 fires per weekday (no weekend runs, no second daily batch); `price_checks` accumulates 63/weekday; `recommendation_outcomes` grew **693 → 2,118** as the 06-12+ recs matured; and the live `phase × action` split confirms the enum holds — HOLDING returns only {HOLD, SELL}, WATCHLIST only {BUY, WATCH, AVOID}, **zero out-of-set actions**.
...
- **API credit balance is exhausted.** A full `--dry-run` mid-session 09 returned `400 invalid_request_error: "Your credit balance is too low"` after one successful run drained the remainder. Top up at the Anthropic Console before the next cron run, or the production run will fail at the first Claude call.
...
- **Wave 3 (observability & hygiene) — complete.** All items landed (session 09). `feat/session-09-wave-3` pushed, **awaiting merge**. Next up is **Wave 4** (action-flip detection in the summary prompt, persist trending-unknown tickers, batched Reddit sentiment [gated on creds], weekly retrospective S5). The S15/S16 quick wins are done.
...
- [ ] **More unit tests.** Still uncovered: `extract_ticker_mentions` stopwords (`IT`, `GO`, `BE`), `_compute_rsi`/`_pct_change` fixtures, per-run dedup window (Wave 1). The **CI step itself** landed session 09 ([.github/workflows/tests.yml](.github/workflows/tests.yml)); these extra cases are the remaining gap. (action-per-phase is covered by [tests/test_actions.py](tests/test_actions.py); `grade()` by [tests/test_outcomes.py](tests/test_outcomes.py).)
...
- [~] **Action-flip detection.** The **dashboard table** half landed early in session 09 (S16): panel-9 is now a day-over-day "Action flips vs previous run" table. **Still TODO:** feed the flips into the daily-summary prompt input so the summary text calls them out. (This is the no-notifications substitute.)
